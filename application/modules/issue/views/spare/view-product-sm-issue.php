...
<h3 class="panel-title" style="float: initial;">Work Order Details:-  WO<?=$getsched->id."&nbsp;&nbsp;SM".$getsched->schedule_id;?>
	...
	<a href="<?=base_url('issue/SpareIssue/add_spare_sm_issue?id=');?><?=$getsched->id?>" class="btn  btn-sm pull-right" type="button"><i class="icon-left-bold"></i> back</a>
</h3>
</div>
...
<div id="spareIssue" class="modal fade modal" role="dialog">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-spare-sm-issue" id="modal-spare-sm-issue">
...
function viewSpareIssue(v,x,y)
{
  ...
  xhttp.open("GET", "issue_spare_sm?PID="+ prod + "&"+"HID="+ sphd + "&"+"WID="+ word, false);
  // xhttp.open("GET", "issue_spare_sm?ID="+pro, false);
  // alert(xhttp.open);
  xhttp.send();
  //alert(xhttp.responseText);
  document.getElementById("modal-spare-sm-issue").innerHTML = xhttp.responseText;
...
function qtyfunc(v) 
{
  ...
  {   
   ...
   //alert(total);
    ...
    var eqty = $("#spare_qty"+asx).val();
    var sqty = $("#stk_qty"+asx).val();

    var rqty  = $("#reqstQty").val();
    var rmQty = $("#remainQty").val();
    //alert(sqty);

    //if(sqty < eqty)
    if(Number(eqty) > Number(sqty))
    {
      alert("Issue Qty Can Not Be Greater Than Stock Qty");
      $("#issueButton").attr('disabled',true);
    }
    //else if(rmQty < total)
    else if(Number(total) > Number(rmQty))
    {
