<?php
class model_master_tools extends CI_Model {
...
function tool_get($last,$strat)
{
	$query=("select *from tbl_product_stock where type = 'tool' and status='A' Order by Product_id DESC limit $strat,$last ");
	$getQuery = $this->db->query($query);
	return $result=$getQuery->result();
}


function filterListtool($perpage,$pages,$get)
{
	...
	if(sizeof($get) > 0)
	{
		  ...
		  if($get['type_of_spare'] != "")
			   {
			     ...
			     if($get['priority'] != "")
			   {
				   $unitQuery=$this->db->query("select * from tbl_master_data where keyvalue LIKE '%".$get['priority']."%' and param_id='27'");
				   $getUnit=$unitQuery->row();
				   $sr_no=$getUnit->serial_number;
			 
				   $qry .= " AND priority ='$sr_no'";
		...
		if($get['quantity'] != "")
			$qry .= " AND quantity LIKE '%".$get['quantity']."%'";
...
function mod_viewItem($id)
{ 

		///tbl_master_data L  find_in_set(M.location,L.serial_number)
...
function getToolsWarrantyData()
{

	$this->db->select("*")
	->from("tbl_tools_warranty")
	->where("status", 'A')
	->where("tools_id",$_GET['id']);
