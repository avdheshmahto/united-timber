...
$machinQuery=$this->db->query("select *from tbl_machine where id='".$_GET['id']."'");
...
<h3 class="panel-title" style="float: initial;">Machine:-<?=$getMachine->machine_name."(".$getMachine->code.")";?>
	
	<a href="<?=base_url('assets/machine/manage_machine');?>" class="btn  btn-sm pull-right" type="button"><i class="icon-left-bold"></i> back</a>
</h3>
</div>
<div class="panel-body">
<form role="form">
...
<h4>Machine Name</h4>
<input type="text" name="" value="<?=$getMachine->machine_name;?>" id="first_name" class="form-control" readonly >
</div>
</div>
<div class="col-xs-6 col-sm-6 col-md-6">
<div class="form-group">
<h4>Machine Code</h4>
<input type="text" name="" id="last_name" class="form-control" value="<?=$getMachine->code;?>" readonly >
</div>
</div>
</div>
...
<div class="form-group">
<?php 
$queryType=$this->db->query("select *from tbl_category where id='$getMachine->m_type'");
$getType=$queryType->row();

?>
<input type="text" name="" value="<?=$getType->name;?>" id="facility" class="form-control" readonly>
</div>
</div>
...
<div class="form-group">
<?php 
$queryTypeunit=$this->db->query("select *from tbl_master_data where serial_number='$getMachine->m_unit'");
...
<textarea class="form-control" readonly><?=$getMachine->machine_des;?></textarea>
</div>
</div>
...
<!-- <li class="active"><a href="#spare" data-toggle="tab">Parts & Supplies</a></li> -->
...
<tr class="gradeU record">
    ...
    <td>
	<a class="modalMapSpare" data-a="<?php echo $getSpareD->Product_id;?>" href='#editItem' onclick="getEditItemm('<?php echo $getSpareD->Product_id;?>','view')" data-toggle="modal" data-backdrop='static' data-keyboard='false' title="View Spare Details">
	
	<?=$getSpareD->productname;?></a></td>
	 <td><?php 
		 $compQuery = $this -> db
		           -> select('*')
   ...
   <td><?php //$pri_col='id';
    ...
    <?php if($view!=''){ ?>
	 ...
	 <button class="btn btn-default delbutton" id="<?php echo $fetch_list->id."^".$table_name."^".$pri_col ; ?>" type="button" title="Delete Spare"><i class="icon-trash"></i></button>	
	<?php }?>
	</td>
...
<tr class="gradeU">
<td>
 <button  class="btn btn-default modalMapSpare" data-a="<?php echo $fetch_list->id;?>" href='#mapSpare'  type="button" data-toggle="modal" data-backdrop='static' data-keyboard='false' formid = "#mapSpareForm" id="formreset" title="Add Spare And Reading Value"><img src="<?=base_url();?>assets/images/plus.png" /></button>
</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
...
$i=1;
$macwarrantyName=$this->db->query("select * from tbl_machine_warranty where warranty_log_id='".$_GET['id']."' and module_type = 'Machine' ");
...
<?php
 $triggerlog=$this->db->query("select * from tbl_schedule_triggering_log where machine_id = '".$_GET['id']."' ");
...
$i=1;
$supplieraName=$this->db->query("select * from tbl_schedule_maintain where machine_name='".$_GET['id']."' ");
foreach($supplieraName->result() as $fetch_list)
{

$contactQ=$this->db->query("select * from tbl_master_data where serial_number='$fetch_list->wostatus' AND param_id='29'");
$contactlist=$contactQ->row();

$contactgroup=$this->db->query("select * from tbl_master_data where serial_number='$fetch_list->maintyp' AND param_id='31'");
$listcontactgroup=$contactgroup->row();
...
<tr class="gradeU record">
	...
	<td><?=$contactlist->keyvalue;?></td>		 
	<td><?=$listcontactgroup->keyvalue;?></td>
...
<h4 class="modal-title">Map Parts And Supplies  &nbsp;&nbsp;<p id="msgSpare" style="color: #F00;" ></h4>
...
<tr class="gradeA">
<input type="hidden" name="machine_id" id="machine_id" value="<?php echo $_GET['id']; ?>">
...
function addMachineWarranties(v){

var pro=v;
 var xhttp = new XMLHttpRequest();
 
  xhttp.open("GET", "getMachineWarranties?ID="+pro, false);
  xhttp.send();
  
  document.getElementById("modal-machinewarranties").innerHTML = xhttp.responseText;
 } 	

 function addMachineSuppliers(v){

var pro=v;
 var xhttp = new XMLHttpRequest();
 
  xhttp.open("GET", "getMachineSuppliers?ID="+pro, false);
  xhttp.send();
...
function saveData()
{
	...
	document.getElementById("msgSpare").innerHTML="Added Successfully";
	setTimeout(function(){
...
alert("Please Select Warranty Usage Term Type");
 ...
 var xhttp = new XMLHttpRequest();
 xhttp.open("GET", "insert_machine_warranty?machine_id="+machine_id+"&warranty_type="+warranty_type+"&provider="+provider+"&wrty_term_type="+wrty_term_type+"&meter_limit="+meter_limit+"&meter_reading_units="+meter_reading_units+"&expiry_date="+expiry_date+"&certificate_no="+certificate_no+"&desc="+desc+"&date_added="+date_added, false);
 xhttp.send();
