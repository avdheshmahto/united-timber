...
function getmeter_currentData(){
	...
	$this->db->select("*")
	->from("tbl_machine_spare_unit_map")
	->where("status", 'A')
	->where("machine_id",$_GET['pri_id_meter']);
...
function getScheduleData($last,$strat){
	...
	$query=("select *from tbl_schedule_maintain where status='A' Order by id DESC limit $strat,$last ");
	$getQuery = $this->db->query($query);
    return $result=$getQuery->result();
}	
