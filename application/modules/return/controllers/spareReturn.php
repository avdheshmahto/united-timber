...
public function stock_refill_qty($qty,$main_id,$loc,$rack_id,$vendor_id,$type,$purchase_price)
{
	...
	$this->db->select('*');
	$array = array('module_status' => $type, 'loc' => $loc, 'rack_id' => $rack_id,'product_id' => $main_id,'supp_name' => $vendor_id,'purchase_price' => $purchase_price);
	$this->db->where($array);
...
public function ajex_rackData()
{
    ...
    <option value="<?php echo $getTypeQuery->id;?>" <?php if($rackId ==$getTypeQuery->id ){echo 'selected';}?>>   <?php echo $getTypeQuery->rack_name;?></option>
     <?php }
