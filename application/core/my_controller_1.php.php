 ...
 function delete_bincard_data() {
	...
	$this->load->model('Model_admin_login');
		...
		foreach($binDtl->result() as $getDtl)
		{
			...
			$this->db->query("DELETE FROM tbl_software_stock_log WHERE log_type='Receipt' and log_id='$id' and vendor_id='$vendor_id' and product_id='$main_id' ");
		}
...
public function delete_updata_stock($qty,$main_id,$loc,$rack_id,$vendor_id,$type,$purchase_price)
{
	...
	if($num>0)
	{
		...
		$sqlProdLoc1="DELETE FROM tbl_product_serial_log WHERE quantity ='$qty',product_id='$main_id',loc='$loc',rack_id='$rack_id',type='opening stock',name_role='bincard opening stock',module_status='$type',supp_name='$vendor_id',purchase_price='$purchase_price'";
